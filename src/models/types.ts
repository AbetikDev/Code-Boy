...
export type ClientMessage = { type: 'ready' } | { type: 'action'; action: Action } | { type: 'room'; room: RoomTheme } | { type: 'command'; command: 'stats' | 'room' | 'settings' | 'gallery' | 'context' | 'toggleOverlay' } | { type: 'debug'; state?: CharacterState; mood?: number; energy?: number; random?: boolean; animation?: string; fps?: number };
